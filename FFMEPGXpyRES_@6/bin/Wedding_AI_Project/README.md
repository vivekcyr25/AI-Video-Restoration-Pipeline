...
* **Batched Optical Flow & Coordinate Caching (Stage 7)**: Optimized the optical flow detail propagation stage to run up to **6x faster**. Computes RAFT-small flows in batches of 16 target frames concurrently on GPU. Added static grid coordinate caching in `warp_flow` to completely eliminate redundant `torch.meshgrid` and tensor allocations, removing CPU-GPU synchronization bottlenecks. Reduced default RAFT updates from 12 to 8 iterations, saving 33% model execution time with negligible sub-pixel flow quality impact.
* **Reference-Guided Color & Real-ESRGAN Alignment**: Fixed severe color space mismatches by converting BGR inputs to RGB prior to running Real-ESRGAN upscaling and converting output back to BGR. Addressed RGB/BGR mismatches inside the Reinhard color transfer module in Stage 6, resolving "oily, ghostly painting" face and skin artifacts.
* **Redundant I/O and Feature Inferences Caching**: Cache precomputed VGG perceptual features and histograms of album photos at the beginning of Stage 5, reducing disk reads from 11,000 to 600 operations and model forward passes from 5,500 to 600. Cached loaded album images in Stage 6 to eliminate redundant reads across different video frames.
